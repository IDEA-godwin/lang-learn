...
function Home() {
  ...
  const { currentStreak, wordsLearnedToday, dailyGoal, progressPercent } = useStreak()
  const wordsRemaining = Math.max(0, dailyGoal - wordsLearnedToday)

  // SVG circle math for progress orb
  const mobileR = 28
  const mobileCirc = 2 * Math.PI * mobileR
  const mobileOffset = mobileCirc - (mobileCirc * progressPercent) / 100

  const desktopR = 45 // percentage-based
  const desktopCirc = 282.6
  const desktopOffset = desktopCirc - (desktopCirc * progressPercent) / 100
      ...
      <main className="flex-1 px-4 pb-32 space-y-8">
        ...
        <section className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {!authLoading && user ? (
            <>
              ...
              <div
                ...
                <div className="space-y-4 md:space-y-2 z-10 w-full">
                  <div>
                    ...
                    <h2 className="font-headline text-3xl font-extrabold text-primary">
                      {progressPercent >= 100 ? 'Goal Complete! 🎉' : 'Keep the streak!'}
                    </h2>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="relative flex items-center justify-center md:hidden shrink-0">
                      <svg className="w-16 h-16 transform -rotate-90" viewBox="0 0 64 64">
                        <circle className="text-tertiary-container" cx="32" cy="32" fill="transparent" r={mobileR}
                          stroke="currentColor" strokeWidth="8"></circle>
                        <circle className="text-tertiary" cx="32" cy="32" fill="transparent" r={mobileR} stroke="currentColor"
                          strokeDasharray={mobileCirc} strokeDashoffset={mobileOffset} strokeLinecap="round" strokeWidth="8"></circle>
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="font-headline text-sm font-bold">{progressPercent}%</span>
                      </div>
                    </div>
                    ...
                    <p className="font-body text-sm md:text-base text-on-surface-variant max-w-[200px]">
                      {progressPercent >= 100
                        ...
                        : `You are only ${wordsRemaining} words away from your daily target.`
                      }
                    </p>
                  </div>
                ...
                <div className="hidden md:flex relative items-center justify-center p-4">
                  ...
                  <svg className="w-32 h-32 md:w-40 md:h-40 transform -rotate-90">
                    <circle className="text-tertiary-container" cx="50%" cy="50%" fill="transparent" r={`${desktopR}%`}
                      stroke="currentColor" strokeWidth="12"></circle>
                    <circle className="text-tertiary" cx="50%" cy="50%" fill="transparent" r={`${desktopR}%`} stroke="currentColor"
                      strokeDasharray={desktopCirc} strokeDashoffset={desktopOffset} strokeLinecap="round" strokeWidth="12"></circle>
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="font-headline text-2xl font-bold">{progressPercent}%</span>
              ...
              <div
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-headline text-2xl font-bold">{currentStreak} {currentStreak === 1 ? 'Day' : 'Days'}</div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-headline text-2xl font-bold">{wordsLearnedToday}</div>
                    <div className="font-label text-xs text-on-surface-variant">Words Today</div>
                  </div>
                </div>
              </div>
            </>
            ...
            <div className="md:col-span-3 bg-surface-container-low rounded-lg p-8 flex flex-col items-center text-center gap-5 relative overflow-hidden">
              ...
              <div className="space-y-2">
                <h2 className="font-headline text-2xl font-extrabold text-on-background">Track your progress</h2>
                <p className="font-body text-sm text-on-surface-variant max-w-xs mx-auto">
                  Sign in to set daily goals, build learning streaks, and monitor your vocabulary growth.
                </p>
              </div>
              <div className="flex flex-wrap gap-3 justify-center">
                <Link
                  to="/signin"
                  className="bg-primary text-on-primary font-label px-6 py-2.5 rounded-full font-semibold hover:bg-primary-dim transition-all shadow-sm"
                >
                  Sign In
                </Link>
                <Link
                  to="/signup"
                  className="bg-surface-container-lowest border-2 border-outline-variant/30 text-on-surface font-label px-6 py-2.5 rounded-full font-semibold hover:bg-surface-container transition-all"
                >
                  Create Account
                </Link>
              </div>
